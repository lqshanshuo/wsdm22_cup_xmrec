...
def param_init(weight, bias):
    nn.init.kaiming_uniform_(weight, a=math.sqrt(5))
    fan_in, _ = nn.init._calculate_fan_in_and_fan_out(weight)
    bound = 1 / math.sqrt(fan_in) if fan_in > 0 else 0
    nn.init.uniform_(bias, -bound, bound)

class MMoEOld(DNNBase):
    def __init__(self, config):
        super(MMoEOld, self).__init__(config)
        ...
        self.u_gate_kernels = nn.ParameterList([nn.Parameter(torch.randn(self.latent_dim, config['num_shared_experts']), requires_grad=True) for i in range(config['num_tasks'])])
        self.u_gate_bias = nn.ParameterList([nn.Parameter(torch.randn(config['num_shared_experts']), requires_grad=True) for i in range(config['num_tasks'])])
        ...
        self.i_gate_kernels = nn.ParameterList([nn.Parameter(torch.randn(self.latent_dim, config['num_shared_experts']), requires_grad=True) for i in range(config['num_tasks'])])
        self.i_gate_bias = nn.ParameterList([nn.Parameter(torch.randn(config['num_shared_experts']), requires_grad=True) for i in range(config['num_tasks'])])
